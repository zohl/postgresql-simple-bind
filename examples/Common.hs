...
import Database.PostgreSQL.Simple.Bind (PostgresBindOptions(..), PGFunction(..), ReturnType(..), PGIdentifier(..))
...
mkFunctionName = convertCase Snake Camel . ("sql_" ++) . pgiName . pgfIdentifier
...
bindOptions = (def :: PostgresBindOptions) {
  ...
  , pboIgnoreFiles     = isInfixOf "nvl"
    ...
    isNullable :: PGIdentifier -> String -> Bool
